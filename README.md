...
- すでに増えてしまった古い heartbeat は、管理者画面で「開催終了」または「大会番号をリセット」を実行すると削除されます。緊急時は `/api/session` に `{"action":"cleanupMatchHeartbeats","adminPassword":"管理者パスワード"}` を POST して、`match:` から始まる heartbeat だけを削除できます。
